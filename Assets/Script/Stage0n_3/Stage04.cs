...
public class Stage04 : MonoBehaviour
{
    ...
    {
        if (EventSystem.current.currentSelectedGameObject.tag == "N_IceAnimals" && Click == false)
        {
        ...
        if (EventSystem.current.currentSelectedGameObject.tag != "CorrectAnimal" && EventSystem.current.currentSelectedGameObject.tag != "N_IceAnimals" && Click == false)
        {
            Click = true;
