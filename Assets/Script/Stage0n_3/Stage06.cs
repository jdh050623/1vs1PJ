...
public class Stage06 : MonoBehaviour
{
    ...
    {
        if (EventSystem.current.currentSelectedGameObject.tag == "S_IceAnimals" && Click == false)
        {
        ...
        if (EventSystem.current.currentSelectedGameObject.tag != "CorrectAnimal" && EventSystem.current.currentSelectedGameObject.tag != "S_IceAnimals" && Click == false)
        {
            Click = true;
